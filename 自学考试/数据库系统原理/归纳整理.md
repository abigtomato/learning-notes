...
* 1974年IBM推出SQL，在systemr实验系统上实现；
* 1986年ANSI颁布了最高的SQL标准；
* 1987年ISO采纳为国际标准；
...
* 不是某个数据库厂商的专有语言，几乎所有的关系型数据库管理系统都支持SQL；
* 简单易学；
* 是一种强有力的语言，灵活使用其语言的要素，可以实现非常复杂和高级的数据库操作；
...
* 数据定义语言DDL：
  * 用于对数据库以及数据库对象进行**创建、删除、修改**等操作，数据库对象一般包含**表、视图、索引、触发器、存储过程**等；
  * create创建数据库或数据库对象；
  * drop删除数据库或数据库对象；
  * alter修改数据库或者数据库对象。
* 数据库操纵语言DML：
  * 用于操作数据库中的各个对象，主要是**查询和更新**操作；
...
* 数据库控制语言DCL：
  ...
  * grant用于授予权限；
  * revoke用于回收权限。
* 嵌入式和动态SQL规则：规定了SQL在高级程序设计语言中的使用；
* SQL的调用和会话规则：包括SQL例程和调用规则。
...
* 使用mysql数据库系统构建开发环境，基本使用两种架构：
  ...
  * **WAMP：Windows + Apache + Mysql + Php/Prel/Python**。
* 启动服务：`mysqld`；
* 用户登录：`mysql -u root -p`；
* 退出登录：`exit`。
...
* 变量：
  ...
  * 分类：
    ...
    * 局部变量：declare var_name var_type default value;（用于在存储过程和存储函数中使用）；
    * 给变量赋值：set @a=10；查询变量的值：select @a。
* 运算符：
  * 算术运算符：`+ - * / %`；
  * 位运算符：``& | ~ ^ >> <<``；
  * 比较运算符：``> < >= <= = <> != <=>``；
...
* 表达式：由**常量、变量、运算符、列名、函数、复杂计算**的组成，一个表达式通常可以得到一个值，这个值和变量或者常量一样，也是具有某种数据类型的，可能的类型有字符串，数值，时间日期型。
* 内置函数：
...
* 查看数据库：
  ...
  * 基本语法：``show {databases|schemas} [like 'patterm']``；
...
* 创建基本表： 
  ...
  * `create [temporary] table tb_name(列名 类型 [列级完整性约束] [默认值], 列名 类型 [列级完整性约束] [默认值] ... [, 表级完整性约束])(engine=innodb)`；
  ...
  * auto_increment关键字：用于指定一个整型的列为自增列，编号从1开始，这个自增的值也是可以被覆盖的；
...
* 修改表：
  ...
  * 修改列change：
    * 作用：用于修改列的**名称**和列的**类型**； 
    * 语法：`change column old_column_name new_column_name new_column_type`；
    * 案例：`alter table stuinfo change column sedpt sdept int(8);`；
    * 注意：在修改列的数据类型时，一定要记得是兼容的，如果不兼容SQL命令不会执行，系统会提示出错，在兼容的情况下，该列的数据可能会被截断，如varchar(10)变为char(1)。
  * 修改列alter：
    * 作用：用于修改列的**默认值**；
    * 语法：`alter column column_name set default 'new_default_value'`；
    * 案例：`alter table customer alter column cust_city set default 'Beijing';`。
  * 修改列modify：
    * 作用：用于修改列的**数据类型**，不管列的名称，同样可以修改列的位置，使用alter或者first即可；
    * 语法：`modify column column_name new_column_type`；
    * 案例：`alter table customer modify column cust_name char(20) not null first;`。
  * 删除列：
    * 语法：`drop [column]`；
    * 案例：`alter table customer drop column cust_phone;`。
  * 重命名表：
    * 语法：`rename to`；
    * 案例：`alter table customer rename to cust;`。

* 删除表：

  * 语法：`drop table [if exists] tb_name [restrict|cascade]`；
  * 案例：`drop table test`；
...
* 创建索引：
  ...
  * 方式一：create index在已有的表上创建索引，这种方式只能创建普通索引和唯一索引，不能创建主键；
...
* 删除索引：
  ...
  * 通过alter table删除索引、主键、外键：
    * 删除索引：`alter table seller drop index index_seller_name`；
...
* 使用insert into ... values插入数据：
  ...
  ```SQL
  insert into tb_name values(值)[,...(值)]
  没有指定列名，默认是往所有列中插入数据
...
* 使用 insert  into ... set插入数据：
  ...
  ```SQL
  insert into customer set cust_name = '张三', cust_city = '广州市', cust_sex = default;
  ```

* 使用insert into ... select插入数据：

  ```SQL
  insert into stuinfo1(sno, sname, sgender, sage,sdept) select * from stuinfo;
  ```

### 4.4.2.删除数据

* 语法：`delete from tb_name [where conditions]`；
* 案例：删除customer表中王五的信息`delete from customer where cust_name='王五'`。

### 4.4.3.修改数据

* 语法：`update tb_name set col_name=value[,col_name=value] [where conditions] [order by...] [limit...]`；
* 案例：`update customer set cust_city = '武汉市' where cust_name = '张三'`。

## 4.5.数据查询

### 4.5.1.SELECT语句基本语法

* 语法：`select [ALL|DISTINCT] {*|column_list} from {table_name|view_name} [where 查询条件] [group by column_name[,...] [having 分组过滤条件]] [order by column_name[,...] [ASC|DESC]] [LIMIT (offset,count)]` ；
...
* having子句：
  ...
  * having和where子句的区别是：
    ...
    * having子句是实现对分组进行过滤，where子句实现对数据行进行过滤；
...
* 创建触发器：
  ...
  * 触发器创建的注意事项：
    ...
    触发器在创建的时候，每个表中的每个事件每次只能创建一个触发器，所以每个表最多可用创建6个；

    即：before insert、after insert、before update、after  update、before delete、after delete；
...
* 数据库安全性的定义：
  ...
  数据库安全性是指保护数据库以防止不合法的使用而造成数据的泄漏、更改或破坏。
...
* 并发控制：
  ...
  多个用户同时操作数据库中相同的数据，如果不采取任何措施，会造成数据库中数据的异常现象，DBMS对这种并发操作提供一定的控制，以防止它们彼此干扰，从而保证数据库的正确性不被破坏，避免数据库的不一致性，这种机制就是并发控制。

* 事务的概念：

  事务是用户定义的一个数据操作序列，这些操作作为一个完整的工作单元，要么全部执行，要么全部不执行，是一个不可分割的工作单位；
...
* 事务的特征（ACID）：
  ...
  1. 原子性（Automicity）：事务是一个不可分割的单位，事务中包含的操作，要么全部执行，要么都不执行；
  2. 一致性（Consistrncy）：事务执行完毕后，将数据由一个一致性状态转变到另一个一致性状态；
  3. 隔离性（Isolation）：要求事务必须是独立的、隔离的，一个事务的执行不受其他事务的干扰，一个事务对数据库的变更结果必须在commit之后，另一个事务才能操作；
  4. 持久性（Durability）：永久性，是指事物一旦提交，对数据库的改变是永久的。

* 并发操作的问题：

  * 丢失更新：即重写问题，两个事务T1，T2，当它们同时读入一个数据并加以修改时，事务T2提交的结果会破坏事务T1提交的结果，由此导致事务T1的修改被丢失；
  * 不可重复读：两个事务T1，T2，不可重复读是指事务T1读取数据后，事务T2随即执行了更新操作，使事务T1无法再现前一次的读取结果；
  * 读脏数据：即读未提交，两个事务T1，T2，读脏数据是指T1事务修改某一个数据，并将其写回磁盘，随后事务T2读取修改后的同一个数据，这时事务T1被回滚，T1修改的数据恢复原样，那么T2读取的那个未被真正提交的数据就是脏数据；
...
* 封锁：
  ...
  * 用锁进行并发控制，即封锁的工作原理：
    * 若事务T对数据D加X锁，则所有别的事务对数据D的锁请求都必须等待事务T释放锁；
    * 若事务T对数据D加S锁，则所有别的事务还可以对数据D请求S锁，而对数据D请求X锁必须等待事务T释放锁；
    * 事务执行数据库操作时，都要先请求相应的锁，对读操作请求S锁，写操作请求X锁，这个过程由DBMS在执行操作的时候隐含的进行；
    * 事务一直占有获取的锁直到事务结束（commit、rollback）时释放。
  * 封锁的粒度：
    * 以粒度去描述封锁数据单元的大小；
    * 粒度越细，并发性越大，系统开销越大；
  ...
  * 封锁的级别：
    ...
    * 0级封锁：封锁的事务不能重写其他非0级事务未提交的更新数据；
    * 1级封锁：被封锁的事务不允许重写未提交的更新数据，避免丢失更新；
    * 2级封锁：被封锁的事务既不重写也不读未提交的更新数据，避免丢失更新和脏读问题；
    * 3级封锁：被封锁的事务不读未提交的更新数据，不写任何未提交的数据，避免丢失更新、脏读、不可重复读。
    * 3级封锁是最严格的封锁，保证了多个事务并发执行的可串行化。
  * 活锁与死锁：
    * 封锁带来的问题：活锁，死锁；
    * 活锁的概念：因为事务的优先级较低永远处于等待状态；
    * 活锁的解决方法：采用先来先服务的调度算法；
    * 死锁的概念：两个或两个以上的事务循环等待被同组中另一个事务锁住的数据单元的情况；
    * 预防死锁的方法：
      * 一次性锁请求：每个事务在处理时，一次性提出对所有要操作数据的锁的请求，仅当这些请求全部满足时，事务才开始执行；
      * 锁请求排序：将每一个数据单元标以线性顺序，所有事务都要按照这个顺序请求锁；
      * 序列化处理：通过应用为每一个数据单元建立一个master程序，对给定数据单元的所有请求，都发送给master，master已单道的形式去运行；
      * 资源剥夺：强行令两个冲突的事务中的一个rollback，释放所有的锁，重新开始。
      * 对死锁的另一个方法就是不去预防，让其发生并随时检测，一旦检测到系统发生了死锁再进行解决，死锁的检测可以用图论的方法实现，并以正在执行的事务为节点。
...
* 第三代数据库技术——新一代数据库系统：
  ...
  * 数据库系统与其他计算机技术融合，是数据库技术发展的显著特征：
    ...
    * 与多媒体技术相结合——多媒体数据库系统；
    * 与并行处理技术相结合——并行数据库系统；
    * 与人工智能技术相结合——演绎数据库、知识数据库和主动数据库系统。
  * 第三代数据系统的应用领域：
    * 工程数据库：应用于工程领域；
...
* 从数据到数据仓库：
  ...
  * 数据仓库的特点：
    ...
    * 数据的时变性。
  * 数据仓库的三个常用的重要概念：
    * 粒度：数据仓库的数据单位中保存数据的细化和综合程度的级别，细化程度越高，粒度级别越小，相反的，细化程度越低，粒度级别越大；
    * 分割：将数据分散到各自的物理单元中，以便能分别处理，提高数据的处理效率。数据分割后的单元称为切片；
    * 维：人们观察数据的一个特定的角度，如：时间维、产品维。
* 数据挖掘技术：
  * 数据挖掘的概念：
  ...
  * 数据挖掘的功能：
    * 概念描述；
    * 关联分析：是数据挖掘中应用较为广泛的方法，包含简单关联、因果关联、时序关联；
    * 分类预测：分类就是找出一个类别的概念描述，代表这类数据的整体信息；
    * 聚类：把数据按照相似性归纳为若干类别，同一类的数据彼此相关，聚类是没有预先的分类特征。常见的聚类算法：k-means，GMM；
    * 孤立点检测：孤立点是指数据中与整体表现行为不一致的数据集合。孤立点往往对错误检测和特例分析是非常有用的；
    * 趋势和演变分析。
  * 数据挖掘的过程实现步骤：
    1. 确定业务对象；
    2. 数据选择；
...
* 大数据的定义：
  ...
  * IBM公司将大数据概括为4个”V“，即：大量化、多样化、快速化、真实性；
