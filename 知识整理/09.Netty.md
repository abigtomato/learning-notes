...
* **IO任务**：即selectionKey代表的就绪事件，如connect、read、write等，由processSelectedKeys方法触发；
...
### ChannelPipeline/ChannelHandler/ChannelHandlerContext关系分析
...
* 这些ChannelHandlerContext用于包装ChannelHandler，并且相互间组成了一个双向链表；
...
### ChannelPipeline/ChannelHandler/ChannelHandlerContext创建过程分析
...
当请求进入时，会调用Pipeline的相关方法，若是入站事件，这些方法则由 fire 开头，表示开始在管道中流动，让后续的Handler依次处理。其中调用的以 invoke 开头的静态方法传入的是 head，即会先调用 head 的 `ChannelInboundInvoker` 接口的方法，然后调用 Handler真正的方法。
...
<img src="assets/image-20201201222047369.png" alt="image-20201201222047369"  />

* 继承自ScheduleExecutorService接口，表示NioEventLoop可以接收定时任务；
* 继承自EventLoop接口是当Channel被注册时用于处理其对应I/O操作的接口；
* 继承自SingleThreadEventExecutor接口，表示NioEventLoop是一个单线程的线程池；
* NioEventLoop是一个单例的线程池，里面包含一个死循环的线程不断的做三件事，即轮询监听、事件处理和队列任务处理。每个EventLoop都可以绑定多个Channel，但每个Channel只能由一个EventLoop处理。
...
* 通过 `select()` 获得感兴趣的事件；
* 通过 `processSelectedKeys()` 处理事件。在select返回后处理事件，并记录I/O事件的处理事件ioTime；
* 通过 `runAllTasks()` 执行队列中的任务。执行任务处理的时间和I/O处理的时间是1:1的关系。
...
* 若任务队列中存在任务，则通过CAS将select状态置为唤醒，然后转为执行非阻塞的 `selectNow()`；
...
processSelectedKeys方法就是对就绪的事件做出响应的。即逐个取出就绪的I/O事件，然后根据事件的具体类型执行不同的策略。
...
* 在Netty的NioEventLoop线程中做耗时的、不可预料的操作时，如：数据库连接、网络请求等，都会严重影响Netty对Socket I/O操作的效率。解决方法就是将耗时任务添加到异步线程池 `EventExecutorGroup` 中去执行；
